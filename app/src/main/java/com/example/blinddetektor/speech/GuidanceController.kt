...
class GuidanceController(private val speech: SpeechManager) {
  ...
  fun start(targetLabel: String) {
    ...
    lastUpdateAt = 0L
    lastSpoken = ""
    speech.speak("Hledám $targetLabel. Hýbej prosím telefonem, dokud se objekt neobjeví v zábìru.")
  }

  fun stop() {
    if (active) speech.speak("Navigace ukonèena.")
    active = false
    target = null
  }
  ...
  fun onDetections(dets: List<Detection>) {
    ...
    if (match == null) {
      if (now - lastPromptAt > 2500) {
        lastPromptAt = now
        speech.speak("Zatím nevidím $t. Pomalu otáèej telefonem do stran.")
      }
      return
    }
    ...
    val action = when {
      match.x2 < 0.45f -> "Otoè trochu doleva."
      match.x1 > 0.55f -> "Otoè trochu doprava."
      match.distanceMeters > 0.6f -> "Pøibliž se."
      else -> "Jsi u nìj."
    }

    val distCm = (match.distanceMeters * 100f).toInt().coerceAtLeast(1)
    val msg = "$t $dir, asi $distCm centimetrù. $action"
